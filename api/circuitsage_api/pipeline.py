"""전체 해석 파이프라인 (design §5) — 워커 프로세스에서 실행 가능한 순수 함수.

parse → 위상 검증 → MNA(유도 기록) → H(s) → pole/zero → 안정성 →
시간응답(+수치 샘플) → Bode → (선택) ngspice 검증 → (선택) LaTeX 노트.
실패해도 성립하는 부분(기호 Bode 등)은 결과에서 빼고 warnings로 알린다.

이 모듈은 라우터/서버를 import하지 않는다: Windows spawn 워커가 이 모듈만
import하므로 자식 프로세스가 서버를 재기동하는 일이 없다 (isolation.py).
입출력은 JSON 직렬화 가능한 dict로만 주고받는다.
"""
...
def run_solve(netlist: str, options: Mapping | None = None) -> dict:
    """Run the full analysis pipeline and return a JSON-serializable dict."""
    opts = dict(options or {})
    numeric_values = opts.get("numeric_values") or {}
    responses_wanted = opts.get("responses") or []
    want_verify = bool(opts.get("verify"))
    want_latex = bool(opts.get("latex"))
    warnings: list[str] = []

    circuit = parse(netlist)
    complexity = measure_complexity(circuit, netlist)
    ...
    try:
        bode = bode_data(tf, numeric_values or None)
        result["bode"] = {
            "freq": list(bode.freq),
    ...
    if want_verify:
        if find_ngspice() is None:
            result["verification"] = {"status": "unavailable", "reports": []}
            warnings.append("ngspice 미설치 — SPICE 교차 검증 생략")
        else:
            try:
                verification_reports = [
                    verify_ac(circuit, numeric_values or None),
                    verify_tran(circuit, numeric_values or None),
                ]
                result["verification"] = {
                    "status": "ok",
                ...
                result["verification"] = {"status": "error", "detail": str(exc), "reports": []}
